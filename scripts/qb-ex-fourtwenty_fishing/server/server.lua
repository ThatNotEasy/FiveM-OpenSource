    ...
    exports.oxmysql:fetchAll('SELECT * FROM fourtwenty_fishing WHERE identifier = @identifier', {
        ...
        if result[1] then
            ...
            if newLevel > result[1].level then
                ...
                if xPlayer then
                    TriggerClientEvent('fishing:levelUp', xPlayer.PlayerData.source, newLevel)
                end
            end
        else
...
AddEventHandler('fishing:attemptCatch', function(rodType)
    ...
    if math.random() <= rod.catchChance then
        ...
        exports['qb-inventory']:AddItem(fish.item, 1)
        AddFishingXP(xPlayer.citizenid, fish.xp * rod.xpMultiplier)
        TrackFishCatch(xPlayer.citizenid, fish.name)
...
QBCore.Functions.CreateCallback('fishing:getRodCount', function(source, cb, rodItem)
    local xPlayer = QBCore.Functions.GetPlayer(source)
    local itemCount = exports['qb-inventory']:GetItemByName(rodItem).count
    cb(itemCount)
end)
